...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (particleContainer) {
        for (let i = 0; i < 18; i++) {
            ...
            p.style.cssText = `
                ...
                background: ${Math.random() > 0.5 ? 'rgba(197,160,89,0.45)' : 'rgba(245,235,224,0.45)'};
            `;
    ...
    window.addEventListener('scroll', () => {
        ...
        navAnchors.forEach(a => {
            a.style.color = a.getAttribute('href') === `#${current}` ? 'var(--khai-gold)' : '';
        });
    });
    ...
    btnSendWhatsapp.addEventListener('click', () => {
        ...
        const message = `*CONFIRMACIÓN DE CITA - KHAI NAIL BAR*%0A%0A` +
                        `✅ *Pago recibido:* ${deposit} (50% anticipo)%0A` +
                        `✨ *Servicio:* ${service}%0A` +
                        `📅 *Fecha:* ${date}%0A` +
                        `⏰ *Horario:* ${time}%0A%0A` +
                        `Su cita ha quedado confirmada en nuestra sucursal de Plaza Fontana. ¡Le esperamos!`;
        
        window.open(`https://wa.me/5219902842312?text=${message}`, '_blank');
    });
