...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (particleContainer) {
        for (let i = 0; i < 18; i++) {
            ...
            p.style.cssText = `
                ...
                background: ${Math.random() > 0.5 ? 'rgba(233,30,99,0.6)' : 'rgba(206,147,216,0.6)'};
            `;
    ...
    window.addEventListener('scroll', () => {
        ...
        navAnchors.forEach(a => {
            a.style.color = a.getAttribute('href') === `#${current}` ? 'var(--rose-gold)' : '';
        });
    });
    ...
    btnSendWhatsapp.addEventListener('click', () => {
        ...
        const message = `*CONFIRMACIÓN DE CITA - NAIL STUDIO*%0A%0A` +
                        `✅ *Pago recibido:* ${deposit} (50% anticipo)%0A` +
                        `💅 *Servicio:* ${service}%0A` +
                        `📅 *Fecha:* ${date}%0A` +
                        `⏰ *Horario:* ${time}%0A%0A` +
                        `Su cita ha quedado confirmada. ¡Le esperamos!`;
